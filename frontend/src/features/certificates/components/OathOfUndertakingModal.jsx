...
import { OathOfUndertakingPreview } from "../templates/oath-of-undertaking/preview.tsx";
...
export default function OathOfUndertakingModal({ isOpen, onClose }) {
  ...
  const [issuedMonthYear, setIssuedMonthYear] = useState("");
  const [cityMunicipality, setCityMunicipality] = useState("Pasay");
  const [witnessName, setWitnessName] = useState("HON. MICHAEL H. LAUZON");
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setIssuedDay("");
      setIssuedMonthYear("");
      setCityMunicipality("Pasay");
      setWitnessName("HON. MICHAEL H. LAUZON");
  ...
  useEffect(() => {
    ...
    const computedAge = calculateAge(selectedResident.birthDate);
  ...
  const canPrint = !!(fullName && address && internalPurpose);

  // ── print handler ────────────────────────────────────────────────────
  const handlePrint = () => {
    logCertificateRequest({
      certificateType: 'OATH_OF_UNDERTAKING',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#322A2C8c] backdrop-blur-[2px] print:absolute print:inset-0 print:bg-white print:block">
      ...
      <div className="bg-white rounded-xs shadow-2xl border border-[#D1D7CE] flex flex-col w-[1050px] max-w-[96vw] max-h-[92vh] overflow-hidden print:w-full print:max-w-none print:h-auto print:max-h-none print:overflow-visible print:border-none print:shadow-none">

        {/* ── Header ────────────────────────────────────────────────────── */}
        <div className="flex items-center justify-between px-6 py-3.5 border-b border-[#D1D7CE] bg-[#FFF8F8] flex-shrink-0 print:hidden">
          <div>
        ...
        <div className="flex flex-1 overflow-hidden print:overflow-visible">
          ...
          <div
            ...
            style={{ width: "380px", minWidth: "350px" }}
          >
            <div className="p-5 space-y-4 flex-1">
              ...
              <div>
                ...
                <div className="relative" ref={dropdownRef}>
                  <div className="relative">
                    <input
                      type="text"
                      className="w-full border border-[#D1D7CE] rounded-xs px-3 py-1.5 text-xs focus:outline-none focus:border-[#322A2C] bg-[#FFF8F8] focus:bg-white text-[#322A2C] placeholder-slate-400"
                  ...
                  {showDropdown && filteredResidents.length > 0 && (
                    <div className="absolute z-20 w-full mt-1 bg-white border border-[#D1D7CE] rounded-xs shadow-lg max-h-52 overflow-y-auto">
                      {filteredResidents.map((r) => (
                        <div
                          key={r.residentId}
                          className="px-3 py-2 text-xs cursor-pointer hover:bg-[#322A2C] hover:text-white transition-colors border-b border-[#D1D7CE]/40 last:border-0"
                          onClick={() => {
              ...
              <div className="border-t border-[#D1D7CE]/60 pt-2">
                <p className="text-[10px] uppercase tracking-wider text-slate-400 font-mono mb-2">
                  Applicant Details
                </p>
                ...
                <FormField label="Residency Start Date">
                  <input
                    type="text"
                    className={inputCls}
                ...
                <FormField label="Address" required>
                  <input
                    ...
                    placeholder="e.g. # 1794 Tramo St., Pasay City"
                  />
                </FormField>

                <p className="text-[10px] uppercase tracking-wider text-slate-400 font-mono my-2 border-t border-[#D1D7CE]/60 pt-2">
                  Signing Details
                </p>

                <div className="grid grid-cols-2 gap-2">
                  <FormField label="Issued Day">
                    <input
                      type="text"
                      className={inputCls}
                      value={issuedDay}
                  ...
                  <FormField label="Month & Year">
                    <input
                      type="text"
                      className={inputCls}
                      value={issuedMonthYear}
                      onChange={(e) => setIssuedMonthYear(e.target.value)}
                ...
                <FormField label="Witness Name">
                  <input
                    ...
                    placeholder="e.g. HON. MICHAEL H. LAUZON"
                  />
                </FormField>

                <FormField label="Witness Position">
                  <input
                    type="text"
                    className={inputCls}
            ...
            {/* ── Form action bar ──────────────────────────────────── */}
            <div className="px-5 py-3 border-t border-[#D1D7CE] bg-[#FFF8F8] flex items-center justify-between gap-3 flex-shrink-0 print:hidden">
              <button
                onClick={onClose}
                className="text-xs font-mono uppercase tracking-wider px-3 py-1.5 border border-[#D1D7CE] text-slate-500 rounded-xs hover:bg-[#FFF8F8] cursor-pointer transition-all"
              >
                Cancel
              </button>
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setStage("preview")}
                  className="text-xs font-mono uppercase tracking-wider px-3 py-1.5 border border-[#322A2C] text-[#322A2C] rounded-xs hover:bg-[#322A2C]/5 cursor-pointer transition-all"
                >
                  Preview →
                </button>
                <button
                  onClick={handlePrint}
                  disabled={!canPrint}
                  className="text-xs font-mono uppercase tracking-wider px-3 py-1.5 bg-[#322A2C] text-white rounded-xs hover:bg-[#0f2436] cursor-pointer transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1.5"
                >
          ...
          >
            ...
            <div className="shadow-2xl origin-top scale-[0.65] mb-[-35%] print:scale-100 print:mb-0 print:shadow-none print:flex print:justify-center print:w-full">
              <OathOfUndertakingPreview ref={printRef} data={previewData} />
            </div>
...
function FormField({ label, children, required = false }) {
  return (
    <div className="mb-2">
      <label className="block text-[10.5px] font-mono uppercase tracking-wider text-slate-500 mb-0.5">
        {label}
...
const inputCls =
  "w-full border border-[#D1D7CE] rounded-xs px-2.5 py-1 text-xs focus:outline-none focus:border-[#322A2C] bg-[#FFF8F8] focus:bg-white text-[#322A2C] placeholder-slate-400 transition-colors";
