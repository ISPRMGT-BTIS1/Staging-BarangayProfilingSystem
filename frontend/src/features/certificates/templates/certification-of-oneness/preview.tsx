...
export interface CertificationOfOnenessData {
  /** Full name of the person being certified — e.g. "Juan Dela Cruz" */
  name: string
  /** Second name or alias — e.g. "Juana Dela Cruz" */
  aliasName: string
...
interface Props {
  data: CertificationOfOnenessData
...
export const CertificationOfOnenessPreview = React.forwardRef<
  HTMLDivElement,
  Props
>(({ data }, ref) => {
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-10 mt-6">
        ...
        >
          {isTagalog ? "SERTIPIKO NG PAGKAKAISA" : "CERTIFICATE OF ONENESS"}
        </h1>
      </div>
        ...
        {isTagalog ? (
          <>
            <p className="indent-12">
              Ito ay nagpapatunay na ang mga pangalang{' '}
              <strong className="uppercase underline decoration-1 underline-offset-4 font-bold">
                {data.salutation ? `${data.salutation} ` : ''}
                {data.name || "[PANGALAN 1]"}
              </strong>{' '}
              at{' '}
              <strong className="uppercase underline decoration-1 underline-offset-4 font-bold">
                {data.aliasName || "[PANGALAN 2]"}
              </strong>{' '}
              ay tumutukoy sa isa at parehong tao, na{' '}
              <span className="italic">nasa hustong gulang, {data.nationality || 'Pilipino'}, at</span>{' '}
              kasalukuyang naninirahan sa <strong className="italic">{data.address || "[TIRAHAN]"}</strong>. Siya ay isang lehitimong residente ng Barangay na ito.
            </p>

            <p className="indent-12">
              Pinatutunayan din na kilala ng lumagda sa ibaba na ang parehong pangalan ay ginagamit ng iisang indibidwal sa kanyang mga opisyal na dokumento at rekord.
            </p>
          ...
          <>
            <p className="indent-12">
              ...
              and{' '}
              <strong className="uppercase underline decoration-1 underline-offset-4">
                {data.aliasName || "[NAME 2]"}
              </strong>{' '}
              refer to one and the same person, who is{' '}
              <span className="italic">of legal age, {data.nationality || 'Filipino'}, and</span>{' '}
              presently residing at <strong className="italic">{data.address || "[ADDRESS]"}</strong> and a resident of this Barangay.
            </p>

            <p className="indent-12">
              This further certifies that it is known to the undersigned that both names are used by the same individual in their official documents and records.
            </p>
            ...
            <p className="indent-12">
              ...
              {data.purpose && data.purpose.trim() !== "" && (
                <span> <strong>({data.purpose})</strong></span>
              )}
              .
            </p>
...
CertificationOfOnenessPreview.displayName = 'CertificationOfOnenessPreview'
